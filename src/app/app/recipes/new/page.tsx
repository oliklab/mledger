...
import { RecipeStore, RecipeMaterial } from '@/storage/recipes';
...
export default function NewRecipePage() {
  ...
  const [recipeDetails, setRecipeDetails] = useState({
    name: `New Recipe - ${new Date().toLocaleDateString('en-IE')}`,
    yield_quantity: '1',
    yield_unit: '',
    notes: '',
  });
  const [lineItems, setLineItems] = useState<LineItem[]>([
    { id: crypto.randomUUID(), material_id: '', quantity: '', details: '' }
  ]);
  ...
  const loadMaterials = useCallback(async () => {
    try {
      ...
      setError('Could not load your materials. Please refresh the page.');
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: "Recipe Created!", description: `"${recipeDetails.name}" has been added to your cookbook.` });
