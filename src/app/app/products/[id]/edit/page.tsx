...
export default function EditProductPage() {
  ...
  // Fetch product data and available recipes
  const loadData = useCallback(async () => {
    if (!productId) return;
    try {
      const supabase = await NewSPASassClient();
      const [productData, recipesData] = await Promise.all([
        new ProductStore(supabase).Read(productId),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: "Product Updated!", description: `"${productDetails.name}" has been successfully saved.` });
  ...
  return (
    <div className="p-4 md:p-8 max-w-4xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Modify the details of this sellable item.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end items-center gap-4">
          {error && !initialLoading && <Alert variant="destructive" className="py-2 px-3 text-sm flex-1"><AlertCircle className="h-4 w-4 mr-2" />{error}</Alert>}
